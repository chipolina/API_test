...
def test_get_characters(api):
    ...
    :param api: фикстура из файла conftest.py, срабатывающая на каждый запуск теста. В параметрах записывается end point
    :type api: функция
    """
    try:
        with allure.step('Отправили GET запрос'):
            r = api.get('/characters')
...
@pytest.mark.parametrize("name, code", [("Avalanche", 200),
                                        ...
                                        ("Ben_Parker", 400),
                                        ("mark", 400)])
def test_get_character_by_name(api, name, code):
    """
    ...
    with allure.step('Отправляем GET запрос на сервер'):
        r = api.get('/character', params={"name": {name}})
    with allure.step('Проверяем код ответа'):
...
def test_post_create_character(api, name, universe, education, weight, height, identity, code):
    """
    ...
    with allure.step('Отправляем POST запрос на сервер и передаем данные из маркировки Parametrize'):
        r = api.post('/character', headers={'Content-type': 'application/json'},
                         json={"name": name,
                               "universe": universe,
                               "education": education,
                               "weight": weight,
...
def test_post_create_character_wrong_headers(api):
    """
    ...
    with allure.step('Отправялем POST запрос с неправильных headers'):
        r = api.post('/character', headers={'Content-type': 'application'}, json=data)
...
def test_post_create_character_errors(api, name, universe, education, weight, height, identity, code, err):
    """
...
def test_del_new_character(api, name, text, code):
    """
    ...
    with allure.step('Отправляем DELETE запрос на сервер и передаем имя созданной записи'):
        r_del = api.delete('/character', params={"name": name})
    ...
    with allure.step('Отправляем GET запрос на сервер c именем удаленной записи'):
        r_get = api.get('/character', params={"name": {name}})
...
def test_del_unknown_character(api):
    """
    ...
    with allure.step('Отправляем DELETE запрос на сервер и передаем имя несуществующей записи'):
        r = api.delete('/character', params={"name": 'Vovan'})
    with allure.step('Проверяем код ответа'):
...
def test_put_update_character(api, name, num):
    f"""
    ...
    with allure.step('Создаем запись, отправляя POST запрос на сервер'):
        api.post('/character', headers={'Content-type': 'application/json'},
                     json={"name": name,
                           "universe": "Marvel Universe",
                           "education": "High school (unfinished)",
                           "weight": 104,
    ...
    with allure.step('Отправляем PUT запрос на сервер с существующем именем и новым значением "weight"'):
        r_put = api.put('/character', headers={'Content-type': 'application/json'},
                            json={"name": name,
                                  "universe": "Marvel Universe",
                                  "education": "High school (unfinished)",
                                  "weight": num,
